...
CI runs `bun run check`, `validate-env` against [`tests/fixtures/prod-env.env.example`](../fixtures/prod-env.env.example), then `bun run test:e2e`. Bun version: align with `package.json` `engines.bun` (CI pins 1.3.x).
...
Full broker + device simulations remain in `scripts/ota-e2e/` (not CI).
...
Prod env validation fixture: `tests/fixtures/prod-env.env.example` (fake secrets only).
