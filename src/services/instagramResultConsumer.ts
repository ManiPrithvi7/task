...
export class InstagramResultConsumer {
    ...
    private async handleResult(result: FetchResult): Promise<void> {
        ...
        // Change detection (Redis): publish/queue only when followers_count changes.
        // Cache key TTL: 24h
        ...
        if (redisSvc?.isRedisConnected()) {
            try {
                ...
                if (typeof cached === 'number' && !Number.isNaN(cached) && cached === next) {
                    logger.debug('[INSTAGRAM_RESULT] No change detected, skipping publish', { deviceId, followers: next });
                    return;
                }

                await client.set(cacheKey, String(next), { EX: 86400 });
            } catch (err: unknown) {
                logger.warn('[INSTAGRAM_RESULT] Change detection failed (continuing)', {
                    deviceId,
